...
trait GameDef {
   ...
   * the game.
   */
  def startBlock: Block = Block(startPos,startPos) // startPos은 만들어져 있겠지
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = {
      ...
      val u = this.up
      val d = this.down
      List((l,Left),(r,Right),(u,Up),(d,Down))
    }
     ...
     */
    def isLegal: Boolean = terrain(b1) && terrain(b2) // terrain(type Terrain = Pos => Boolean)은 만들어져있겠지.
  }
}
