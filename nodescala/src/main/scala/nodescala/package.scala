...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      cts //unsubscribe 를 호출할 수 있는 객체를 보내자.
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]
      f.onComplete(_ => p.success(cont(f))) // f가 끝나면...
      p.future
    }
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]
      f.onComplete(tryT => p.success(cont(tryT))) //Option[Try[T]]
      p.future
    }
  ...
  object CancellationTokenSource {
    ...
    def apply() = new CancellationTokenSource {
      val p = Promise[Unit]() //시작할때 promise랑 cancellation token을 만들어둔다.
      val cancellationToken = new CancellationToken {
        def isCancelled = p.future.value != None //promise의 future 값이 존재하면 cancelled된것이다.
      }
      def unsubscribe() {
        p.trySuccess(()) // unsubscribe로 promise의 future 값을 설정한다.
      }
    }
  }
}
